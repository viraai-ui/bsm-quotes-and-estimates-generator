  ...
  return <section className="panel line-panel wide">
    ...
    <div className={`line-table ${isEstimate ? 'estimate-lines' : ''}`}>
      ...
      {items.map((item) => { const taxable = item.quantity * item.price; const total = taxable + (settings.tax.gstEnabled ? taxable * ((settings.tax.rowLevelGst ? item.gst : settings.tax.defaultGst) / 100) : 0); return <div className="line-row" key={item.id}>
        ...
        <label className={`mobile-line-field ${isEstimate ? 'field-days' : 'field-qty'}`}><span>{isEstimate ? 'Days' : 'Quantity'}</span><input type="number" value={item.quantity} onChange={(e) => update(item.id, { quantity: Number(e.target.value) })} /></label>
        <label className={`mobile-line-field ${isEstimate ? 'field-cost' : 'field-price'}`}><span>{isEstimate ? 'Cost' : 'Price'}</span><input type="number" value={item.price} onChange={(e) => update(item.id, { price: Number(e.target.value) })} /></label>
        <label className="mobile-line-field field-gst"><span>GST %</span><input type="number" value={item.gst} disabled={!settings.tax.rowLevelGst} onChange={(e) => update(item.id, { gst: Number(e.target.value) })} /></label>
