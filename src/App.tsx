...
function App() {
  ...
  const visibleQuoteFields = settings.quotationFields.filter((f) => f.visible).sort((a, b) => a.sortOrder - b.sortOrder)
...
function downloadQuotationPdf(doc: SavedDocument, settings: Settings) {
  ...
  autoTable(pdf, { startY: 84, head: [['#', 'Picture', 'Product', 'Description', 'Qty', 'Price', 'GST %', 'Total']], body: doc.items.map((item, i) => [String(i + 1), item.image ? 'Image attached' : '-', item.productName, item.description, item.quantity, moneyPlain(item.price), item.gst, moneyPlain(item.quantity * item.price * (1 + item.gst / 100))]), styles: { fontSize: 8, cellPadding: 2 }, headStyles: { fillColor: [17, 24, 39] }, columnStyles: { 3: { cellWidth: 45 } }, didDrawCell: (data) => { if (data.section === 'body' && data.column.index === 1) { const item = doc.items[data.row.index]; if (item?.image) { try { pdf.addImage(item.image, 'JPEG', data.cell.x + 2, data.cell.y + 2, 12, 12) } catch { /* ignore invalid image */ } } } } })
